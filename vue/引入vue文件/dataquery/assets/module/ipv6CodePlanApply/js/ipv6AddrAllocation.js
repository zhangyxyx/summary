$(function(){
    ...
    OriIpTypeChoose = urlObj.OriIpType;
    ...
    $("#PreIpTypeList").combotree('setValue',OriIpTypeName);  
    $("#addrNum").val('1')
    // $(".addrTypeName").html(OriIpTypeName);
    //$(".freeAddr").html(freeAddr);
    //$(".addrPrefixLen").html(freeAddr.split("/")[1]);
    getNodeList();
    getV6IpTypeList(topNodeCode,OriIpType);
    getV6IpTypeListPre(topNodeCode,OriIpType)    
...
var OriIpTypeChoose = '';//原地址类型选择的地址类型
    ...
    $("#submitBtn").click(function(){
        ...
        for(var i = 0;i < ipv6TableList.length;i ++){
            allocationAddrList.push({
                inetnum:ipv6TableList[i].addInetNum,
                nodeCode:topNodeCode,
                nextNodeCode:$("#nodeList").combobox('getValue'),
                ipTypeCode:$("#PreIpTypeList").combobox('getValue'),
        ...
        $.ajax({
            ...
            success:function(obj){
                if(obj.data == ""||obj.data == null){
                    $.messager.alert('提示','地址新增成功！','success',function () {
                        const ifrs = window.top.$vm.$('iframe')
                        for (const item of ifrs) {
                          if (item.src.indexOf('/ipmanage/nos/ipv6manage/allocate/AllocateIpCondition.jsp') != -1) {
                             const AllocateIpList = item.contentWindow.document.getElementById("displayFrame")
                            // item.contentWindow.getIpTypeSchemeList()
                            AllocateIpList.contentWindow.location.reload(true)
                          }
                        }
                    window.top.$vm.$closeTab();
                    });
                }else{
                    $.messager.alert('提示','地址新增失败，'+obj.tip,'error');
                }
            },
...
function automaticAssignBtnClickEvent(){
    $("#automaticAssignAddr").click(function(){
        if($("#PreIpTypeList").combobox('getText')==''){
            $.messager.alert('提示','请先选择原地址类型!','warning');
            return;
        }
        if($("#nodeList").combobox('getText')==''){
            $.messager.alert('提示','请先选择申请组织!','warning');
            return;
        }
        if($("#ipTypeList").combobox('getText')==''){
            $.messager.alert('提示','请先选择新地址类型!','warning');
            return;
        }
        //判断是否选择了自动分配
        ...
        if(addrPrefixLenVal < addrPrefixLen || addrPrefixLenVal > 128){
...
function manuallyAssignedBtnClickEvent(){
    $("#manuallyAssignedAddr").click(function(){
        ...
        for(var q = 0;q < ipv6TableList.length;q ++){
            if(ipv6Addr.split("/")[0] == ipv6TableList[q].initIpv6){
                $.messager.alert('提示','Ipv6地址与地址列表已分配地址冲突!','warning');
                return
            }
        }
        //反校验IPv6地址
        var flag = ipv6AntiCheck(ipv6AddrS);
        if(!flag){
            $.messager.alert('提示','IPv6地址不符合编码方案!','warning');
            return
        }
        //校验是否符合表格中的编码方案
        if(!checkTablePlan(ipv6AddrS)){
            $.messager.alert('提示','IPv6地址不符合编码方案!','warning');
            return
        }
        //调用一个空闲地址接口
            ...
            var PreNodeCode1 = ''
            if(getSysPara() == 'Y'){
                PreNodeCode1 = $("#nodeList").combobox('getValue')
            }else{
                PreNodeCode1 = ''
            }
        AllIpv6List=[]
        AllIpv6List.push(ipv6Addr)
        filterIpv6Info(PreNodeCode1);
        if(AllIpv6List == ''){
            $.messager.alert('提示','IPv6地址不在空闲范围内!','warning');
            return;
        }
        var ipv6AddrS = ipv6AddrFormatStartEnd(ipv6Addr,'start')
        var ipv6AddrE = ipv6AddrFormatStartEnd(ipv6Addr,'end')
        ...
        ipv6TableList.push({
            ...
            OriIpTypeName:$("#PreIpTypeList").combobox('getText'),
...
function ipv6AntiCheck(ipv6Addr){
    var ipv6AddrStart = ipv6AddrFormatStartEnd(ipv6Addr,'start')
    var count = 0;
    var flag = true;
    for(var i = 0;i < schemelist.length;i ++){
        if(schemelist[i].flag == true || schemelist[i].flag == undefined){//已选标识
            if(schemelist[i].LableName == "省份标识"){
                $.ajax({ 
                    ...
                    success:function(obj){
                        ...
                        var ipv6AddrStr = ipv6AddrStart.replace(/[:]/g, "");
...
function getAddrItemBtnClickEvent(){
    $("#getAddrItem").click(function(){
        ...
        if($("#PreIpTypeList").combotree('getText') == ""){
            $.messager.alert('提示','请先选择原地址类型!','warning');
            return;
        }
        ...
        if(getSysPara() == 'Y'){
            var PreNodeCode = $("#nodeList").combobox('getValue')
            window.open(bathPath+"/nos/ipv6manage/unusedipalloc/unusedIpAllotCondition.jsp?allotType=allocate&NodeCode="+topNodeCode+"&inetnum="+freeAddr+"&IpTypeCode="+OriIpTypeChoose+"&ipRange="+ipRangeList.join(';')+"&prenodecode="+PreNodeCode)

        }else{
            window.open(bathPath+"/nos/ipv6manage/unusedipalloc/unusedIpAllotCondition.jsp?allotType=allocate&NodeCode="+topNodeCode+"&inetnum="+freeAddr+"&IpTypeCode="+OriIpTypeChoose+"&ipRange="+ipRangeList.join(';'))
        }
...
// })
    //     window.open("nos/ipv6manage/unusedipalloc/unusedIpAllotCondition.jsp,toolbar=no,location=no,scrollbars=yes,resizable=yes,width=1200,height=700,left=0,top=0");	
    })
// }
}
...
function getIpv6ListByAutomatic(){
    ...
    for(var k=0;k<SplitIpv6AddrArr.length;k++){
        AllIpv6List.push(SplitIpv6AddrArr[k].IPv6Inetnum)
    }
    ...
    AllIpv6List.forEach((value)=>{
        ipv6TableList.push({
            nodeName:$("#nodeList").combobox('getText'),
            addInetNum:value.Ipv6Addr,
            StartFullIp: value.StartFullIp,
            EndFullIp: value.EndFullIp,
            addrPrefixLen:$("#addrPrefixLen").val(),
            OriIpTypeName:$("#PreIpTypeList").combotree('getText'),
            PlanIpTypeName:$("#ipTypeList").combobox('getText'),
            Remark:$("#editRemarks").val(),
...
function SplitIpv6Addr(){
    ...
    if($("#addrPrefixLen").val() >= BitLength){
        ...
        if(AddrNum > 256){
            $.messager.alert('提示','地址个数超过256!','error');
        ...
        for(var i = 0;i<AllIpv6List.length;i++){
                ...
                $.ajax({
                    ...
                    success:function(obj){
                        if(obj.code == 0001){
                            ...
                            AllIpv6ListSplit=AllIpv6ListSplit.concat(obj.data)
                        }
                    },
                    error:function(error){
                        $.messager.alert('提示','接口调用失败!','error');
            ...
            if(AllIpv6ListSplit.length >= $("#addrNum").val()){
                //是否为分配组织的原地址类型下的空闲地址
                if(filterIpv6Info1(PreNodeCode1,AllIpv6ListSplit).length>= $("#addrNum").val()){
                    break;
                }
            }
        }
...
function getSysPara(){
    var PARAVALUE  = ''
    $.ajax({
        url:encodeURI(bathPath+'/ipaddrmodule/SysCommon/getSysPara?ParaName=IPPlanV6'),
        type:'POST',
        data:{},
        async: false,
        dataType:'json',
...
//是否为分配组织的原地址类型下的空闲地址
function filterIpv6Info(PreNodeCode){
    let params = {
        NodeCode:topNodeCode,
        IpTypeCode:$('#PreIpTypeList').combotree('getValue'),
...
//是否为分配组织的原地址类型下的空闲地址
function filterIpv6Info1(PreNodeCode,AllIpv6ListSplit){
    let list = []
    let filterList = []
...
function getNodeList(){
    $.ajax({
        ...
        type:'post',
        cache:false,
        data:JSON.stringify({userName:userName,nodeCode: topNodeCode}),
        dataType:'json',
        contentType:"application/json",
        success:function(obj){                                    
            let nodeList = obj.data;
            var nextNodeList = getNextNodeInfo(nodeList);
            nextNodeList.unshift({
                nodeCode:topNodeCode,
            ...
            $("#nodeList").combobox({
                ...
                onSelect:function(row){
                    getV6IpTypeList(row.nodeCode,OriIpTypeChoose)
                    schemelist=[]
                    // loadLabelCodeTable()
                    ipv6TableList=[]
                    loadIpv6InfoTable()
...
//获取原来的v6地址类型
function getV6IpTypeListPre(nodeCode,OriIpType){
    let params = {
        userName: userName,
        nodeCode: nodeCode,
        ipType: OriIpType,
    ...
    $.ajax({
        ...
        success:function(obj){     
            ...
            $("#PreIpTypeList").combotree({
                ...
                onLoadSuccess: function () {
                    $("#PreIpTypeList").combotree('setValue',OriIpTypeName); 
                    // $("#PreIpTypeList").combotree('tree').tree("collapseAll");
                },
                filter: function(q, row){
                ...
                keyHandler: {
                    query: function(q, e) {
                        $('#PreIpTypeList').combotree('tree').tree('doFilter', q)
                    }
                },
                ...
                onSelect:function(row){
                    OriIpTypeChoose = row.ipTypeId
                    getV6IpTypeList(nodeCode,row.ipTypeId)
                    $("#nodeList").combobox('setValue','')
...
//获取v6地址类型
function getV6IpTypeList(nodeCode,OriIpType){
    let params = {
        userName: userName,
        nodeCode: nodeCode,
        ipType: OriIpType,
    ...
    $.ajax({
        ...
        success:function(obj){     
            ...
            $("#ipTypeList").combotree({
                ...
                onSelect:function(row){
                    // 如果分配组织和申请组织一样的话，新地址类型和原地址类型不能一样
                    if($("#nodeList").combobox('getValue') == topNodeCode && $("#PreIpTypeList").combotree('getValue') == row.ipTypeId){
                        $.messager.alert('提示','如果分配组织和申请组织一样的话，新地址类型和原地址类型不能一样!','error');
                    }else{
                        if(ipv6TableList.length != 0){
                            ipv6TableList=[]
                            loadIpv6InfoTable()
...
function loadLabelCodeTable(){
    ...
    if(count == 0){
        initComboboxDataByNode() // 含有区县，调关联节点
    }else{
        initComboboxData();
    }
    getLevel()
...
//初始化关联节点的下拉数据
function initComboboxDataByNode(){
    for(var i = 0;i < schemelist.length;i++){
        ...
        if(schemelist[i].LableCodeName == undefined||schemelist[i].LableCodeName == ''){//未定义的编码名称
            ...
            $.ajax({
                ...
                success:function(obj){
                    if(obj.data.length != 0){
                        var labelList = obj.data;
                    // 去重
                   let labelNameList = labelList.reduce((cur, next) => {
                        obj[next.LableCodeName] ? "" : obj[next.LableCodeName] = true && cur.push(next);
                        return cur;
                        }, []) //设置cur默认类型为数组，并且初始值为空的数组
                    // if(schemelist[i].LableCodeName != undefined &&schemelist[i].LableCodeName != ''){
                    ...
                    $(domId).combobox({
                        ...
                        onSelect:function(row){
                            var CodeNameList = $(domId).combobox('getValues').join(",");
                            var LableId = domId.slice(14,domId.length)
                            initLabelCodeListByLabelNameByNode(LableId,CodeNameList,labelList)
                            //联动编码方案数据
                            for(var k = 0 ;k < schemelist.length; k++){
                                if(schemelist[k].LableId == LableId){
                                    schemelist[k].LableCodeName = $(domId).combobox('getText')
...
//编码名称改变联动编码数据下拉框(通用标识)(关联节点)
function initLabelCodeListByLabelNameByNode(LableId,CodeNameList,labelList){
    let domId = "#labelCodeList" + LableId;
    // console.log(CodeNameList)
    // console.log(labelList)
    let codeList = []
    labelList.forEach((item,index) => {
        CodeNameList.split(',').forEach((item1,index1) => {
            if(item1 == item.LableCodeName){
                codeList.push(item)
            }
        }) 
    })
    ...
    $(domId).combobox({
        ...
        onSelect:function(row){
            var CodeList = $(domId).combobox('getValues').join(",");
            var LableId = domId.slice(14,domId.length)
            //联动编码方案数据
            for(var k = 0 ;k < schemelist.length; k++){
                if(schemelist[k].LableId == LableId){
...
function loadIpv6InfoTable(ipv6Len){
    /*定义表格的表头*/
    var columnsData=[[
        {field:'nodeName', title: '申请组织',align:'center',width:100},
        {field:'addInetNum',  title: 'IPv6地址',align:'center',width:150},
        {field:'addrPrefixLen', title: '地址前缀长度',align:'center',width:100},
        {field:'OriIpTypeName', title: '原地址类型',align:'center',width:100},
        {field:'PlanIpTypeName', title: '新地址类型',align:'center',width:100},
        {field:'Remark', title: '备注',align:'center',width:100},
        {field:'operate', title: '操作',align:'center',width:50,
        formatter:function(value,row,index){
            return "<a style='color:#f40' class='operateBtn' onclick=delIpv6AddrItem(\'"+row.addInetNum+"\')>删除</a>"
        }},
    ]];
...
//列表ip地址删除
function delIpv6AddrItem(addInetNum){
    ipv6TableList = ipv6TableList.filter(function(ele,idx,arr){
        return ele.addInetNum != addInetNum;
    });
    loadIpv6InfoTable();
}
