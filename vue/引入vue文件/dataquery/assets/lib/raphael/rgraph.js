...
/***/ function(module, exports, __webpack_require__) {
    var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function() {
        ...
        Node.prototype.add = function(id, option) {
            ...
            // console.log("_textAlign=="+_textAlign)
            ...
            switch (_type) {
                case 'circle':
                    ...
                    break;
                case 'ellipse':
                    var _rx = option.rx ? option.rx : 60;
                    var _ry = option.ry ? option.ry : 30;
                ...
                case 'image':
                    _node.rNode = _node.graph.rPaper.image(_src, _x - _width / 2, _y - _height / 2, _width, _height).attr(_attr).toBack();
                    if (_text) {
                        _node.rText = _node.graph.rPaper.text(_x, _y, _text).attr(_textAttr).toFront();
                        var textBBox = _node.rText.getBBox();
                        if ('bottom' == _textAlign) {
                            _node.rText.attr({
                                y: _y + _height / 2 + padding + textBBox.height / 2-5
                            });
                        } else if ('top' == _textAlign) {
                            _node.rText.attr({
                                y: _y - _height / 2 - padding - textBBox.height / 2+5
...
/***/ function(module, exports, __webpack_require__) {
    var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function() {
        ...
        Line.prototype.add = function(n1, n2, option) {
            ...
            _line.isCurve = _isCurve;//将此属性赋值到线上
            // console.log(_isCurve);
            if (!_isCurve) {//直线
            ...
            //扩展线
            var _lineMidElem = option.lineMidElem ? option.lineMidElem : {};
            var _lineMidElemAttr = option.lineMidElemAttr;
             _line.lineMidElemAttr = _lineMidElemAttr;
            if(_lineMidElem.type){
                if(_lineMidElem.type=="circle"){
                    var midPoint = _line.rLine.getPointAtLength(_line.rLine.getTotalLength() / 2);
                    _line.rMid = _line.graph.rPaper.circle(midPoint.x, midPoint.y, 12).attr(_lineMidElemAttr);
               }

            }
            var _text = option.text;
            var _textAttr = option.textAttr ? option.textAttr : {};
            _line.textAttr = _textAttr;
            _line.circuitid = option.circuitid||'';
            if (_text) {
                ...
                //扩展修改
                if(_textAttr['v-align']=="center"){
                    _line.rText.attr({
                        x: cPoint.x,
                        y: cPoint.y
                    });
                }else if(_textAttr['v-align']=="top"){
        ...
        // };
        Line.prototype.rePaint = function() {//线的重绘
            var _line = this;
            ...
            if (_line.rText) {
                var cPoint = _line.rLine.getPointAtLength(_line.rLine.getTotalLength() / 2);
                //扩展修改
                if(_line.textAttr['v-align']=="center"){
                    _line.rText.attr({
                        x: cPoint.x,
                        y: cPoint.y
                    });
                }else if(_line.textAttr['v-align']=="top"){
                    var mathAngle = RMath.transToMathAngle(cPoint.alpha%180);
                    _line.rText.attr({
                        x: cPoint.x - Math.abs(Math.sin(mathAngle) * 20),
                        y: cPoint.y - Math.abs(Math.cos(mathAngle) * 20),
                        transform: 'r' + (cPoint.alpha % 180 > 90 ? cPoint.alpha % 180 - 180 : cPoint.alpha % 180)
                    });
                }
                // var mathAngle = RMath.transToMathAngle(cPoint.alpha%180);
